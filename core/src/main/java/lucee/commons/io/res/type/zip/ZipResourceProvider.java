...
public class ZipResourceProvider implements ResourceProviderPro {
	...
	public ResourceProvider init(String scheme, Map arguments) {
		if (!StringUtil.isEmpty(scheme, true)) this.scheme = scheme;
		if (arguments != null) {
	...
	public Resource getResource(String path) {
		...
		if (index != -1) {
			...
			try {
				return new ZipResource(this, getCompress(file), path.substring(index + 1), caseSensitive);
			}
			catch (IOException e) {
		...
		try {
			return new ZipResource(this, getCompress(file), "/", caseSensitive);
		}
		catch (IOException e) {
			throw ExceptionUtil.toRuntimeException(e);
		}
	...
	public ZipUtil getCompress(Resource file) throws IOException {
		return ZipUtil.getInstance(file, ZipUtil.FORMAT_ZIP, caseSensitive);
	}
