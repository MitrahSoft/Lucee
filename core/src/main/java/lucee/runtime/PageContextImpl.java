...
public final class PageContextImpl extends PageContext {
	...
	public Object getFunctionWithNamedValues(Object coll, Key key, Object[] args) throws PageException {
		if (config.hasDebugOptions(ConfigPro.DEBUG_TEMPLATE) && !gatewayContext) {
			...
			Object result = variableUtil.callFunctionWithNamedValues(this, coll, key, args);
	...
	private final void execute(PageSource ps, boolean throwExcpetion, boolean onlyTopLevel) throws PageException {
		...
		try {
			...
			if (ExceptionUtil.isThreadDeath(t) && getTimeoutStackTrace() != null) {
				t = pe = new RequestTimeoutException(this, (Error) t);
			}
	...
	private void initIdAndToken(boolean createIfNeeded) {
		...
		if (oCfid != null) {
			...
			if (!CFIDUtil.isCFID(this, oCfid)) {
				...
				jakarta.servlet.http.Cookie[] cookies = getHttpServletRequest().getCookies();/* JAVJAK */
				String name, value;
				if (cookies != null) {
	...
	// called by generated bytecode
	public Tag use(String tagClassName, String maven, String fullname, int attrType) throws PageException {
		return use(tagClassName, maven, fullname, attrType, null);
	}
	...
	public Tag use(String tagClassName, String maven, String fullname, int attrType, String template) throws PageException {
		parentTag = currentTag;
		currentTag = tagHandlerPool.use(tagClassName, maven);
		return setup(currentTag, tagClassName, fullname, attrType, template);
	}
	...
	private Tag setup(Tag currentTag, String tagClassName, String fullname, int attrType, String template) throws PageException {
	...
	public void reuse(Tag tag, String maven) {
		currentTag = tag.getParent();
		tagHandlerPool.reuse(tag, maven);
	}
	...
	public void setVariablesScope(Variables variables) {
		...
		// we have a while here for closures in closures
