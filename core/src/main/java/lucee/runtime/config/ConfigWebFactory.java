...
public final class ConfigWebFactory extends ConfigFactory {
	...
	synchronized static void load(ConfigServerImpl cs, ConfigImpl config, ConfigWebImpl cwi, Struct root, boolean isReload, boolean doNew, boolean essentialOnly,
		...
		if (!essentialOnly) {
			boolean madeInstallation = _loadExtensionBundles(cs, config, root, log);
				...
				while (it.hasNext()) {
					try {
...

						{
							...
							else LogUtil.logGlobal(ThreadLocalPageContext.getConfig(configServer == null ? config : configServer), Log.LEVEL_ERROR,
									ConfigWebFactory.class.getName(), "missing cache name");
				...
				while (it.hasNext()) {
					try {
						...
						if (!StringUtil.isEmpty(id)) {
							...
							LogUtil.logGlobal(ThreadLocalPageContext.getConfig(configServer == null ? config : configServer), Log.LEVEL_ERROR, ConfigWebFactory.class.getName(),
									"missing id");
				...
				if (ctMappings.size() > 0) {
					...
					while (it.hasNext()) {
						try {
							...
							else if ("{lucee-config}/customtags/".equals(physical)) hasDefault = true;
					...
					if (!hasDefaultServer) {
						list.add(new MappingImpl(config, "/default-server", "{lucee-config}/customtags/", null, ConfigPro.INSPECT_NEVER, -1, -1, true, false, true, true, false,
								true, null, -1, -1));
					}
					...
					try {
						...
						servers.add(i,
								new ServerImpl(Caster.toIntValue(getAttr(el, "id"), i + 1), getAttr(el, "smtp"), Caster.toIntValue(getAttr(el, "port"), 25),
										...
										toBoolean(getAttr(el, "reuseConnection"), true), hasCS ? ServerImpl.TYPE_LOCAL : ServerImpl.TYPE_GLOBAL));
				...
				try {
					...
					if (cd.hasClass() && !StringUtil.isEmpty(name)) {
						...
						try {
							Class clazz = cd.getClazz();
							Object obj;
							ConstructorInstance constr = Reflector.getConstructorInstance(clazz, new Object[] { configServer }, true);
	...
	private static void _loadScheduler(ConfigServer configServer, ConfigImpl config, Struct root, boolean refresh, Log log) {
		try {
			...
			config.setScheduler(configServer != null ? configServer.getEngine() : ((ConfigServer) config).getEngine(), scheduledTasks, refresh);
		}
		catch (Throwable t) {
			ExceptionUtil.rethrowIfNecessary(t);
			log(config, log, t);
		...
		try {
			...
			if (hasAccess) {
				...
				if (compMappings.size() > 0) {
					...
					if (!hasDefaultServer) {
						list.add(new MappingImpl(config, "/default-server", "{lucee-server}/components/", null, ConfigPro.INSPECT_NEVER, -1, -1, true, false, true, true, false,
								true, null, -1, -1));
					}
		...
		try {
			...
			if (hasAccess && asc.hasClass()) {
				try {
					...
					ExceptionUtil.rethrowIfNecessary(t);
					LogUtil.logGlobal(configServer == null ? config : configServer, ConfigWebFactory.class.getName(), t);
