...
public final class DBUtil {
	...
	public static DataSourceDefintion getDataSourceDefintionForType(Config config, String type, DataSourceDefintion defaultValue) {
		...
		if ("mysql".equals(type)) {
			if (MYSQL == null) {
				JDBCDriver jdbc = getJDBCDriver(config, "mysql", "com.mysql.cj.jdbc.Driver", "com.mysql.cj", "9.3.0", "jdbc:mysql://{host}:{port}/{database}");
		...
		if ("postgresql".equals(type) || "postgre".equals(type)) {
			if (POSTGRESQL == null) {
				JDBCDriver jdbc = getJDBCDriver(config, "postgresql", "org.postgresql.Driver", "org.postgresql.jdbc", "42.7.7", "jdbc:postgresql://{host}:{port}/{database}");
	...
	public static class DataSourceDefintion {
		...
		// Constructor with bundleName and bundleVersion
		DataSourceDefintion(String className, String connectionString, int port, String bundleName, String bundleVersion) {
			this(new ClassDefinitionImpl(className, bundleName, bundleVersion, null), connectionString, port, bundleName, bundleVersion);
		}

		DataSourceDefintion(ClassDefinition cd, String connectionString, int port, String bundleName, String bundleVersion) {
