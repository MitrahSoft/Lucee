...
public final class DataSourceImpl extends DataSourceSupport {
	 ...
	 * @throws SQLException
	 */
	public DataSourceImpl(Config config, String name, ClassDefinition cd, String host, String connStr, String bundleName, String bundleVersion, String database, int port,
			String username, String password, TagListener listener, int connectionLimit, int idleTimeout, int liveTimeout, int minIdle, int maxIdle, int maxTotal,
			long metaCacheTimeout, boolean blob, boolean clob, int allow, Struct custom, boolean readOnly, boolean validate, boolean storage, TimeZone timezone, String dbdriver,
			ParamSyntax paramSyntax, boolean literalTimestampWithTSOffset, boolean alwaysSetTimeout, boolean requestExclusive, boolean alwaysResetConnections, Log log)
	...
	public DataSource _clone(boolean readOnly) {
		try {
			return new DataSourceImpl(ThreadLocalPageContext.getConfig(), getName(), getClassDefinition(), host, connStr, bundleName, bundleVersion, database, port, getUsername(),
					getPassword(), getListener(), getConnectionLimit(), getIdleTimeout(), getLiveTimeout(), getMinIdle(), getMaxIdle(), getMaxTotal(), getMetaCacheTimeout(),
					isBlob(), isClob(), allow, custom, readOnly, validate(), isStorage(), getTimeZone(), dbdriver, getParamSyntax(), getLiteralTimestampWithTSOffset(),
					alwaysSetTimeout, isRequestExclusive(), isAlwaysResetConnections(), getLog());
		}
