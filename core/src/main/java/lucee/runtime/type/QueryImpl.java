 ...
 */
public final class QueryImpl implements Query, Objects, QueryResult {
				...
				if (hasResult) {
					...
					if (fillResult(qry, qr, keyName, dc, stat.getResultSet(), maxrow, true, createGeneratedKeys, tz)) {
						...
						while (stat.getMoreResults() || stat.getUpdateCount() != -1) {
							// continue until no more results (getMoreResults=false AND getUpdateCount=-1)
						}
						break;
					}
				}
		...
		try {
			...
			// fill QueryResult
				...
				while (result.next()) {
					...
					sct = new StructImpl(Struct.TYPE_LINKED, StructImpl.optimalCapacity(usedColumns.length, 2));
					Object val;
	...
	public QueryImpl(String[] strColumns, int rowNumber, String name) {
		...
		for (int i = 0; i < strColumns.length; i++) {
			...
			columns[i] = new QueryColumnImpl(this, columnNames[i], Types.OTHER, getRecordcount());
		}
	}
	...
	public QueryImpl(Array arrColumns, Array arrTypes, int rowNumber, String name) throws PageException {
		...
		for (int i = 0; i < columncount; i++) {
			...
			columns[i] = new QueryColumnImpl(this, columnNames[i], SQLCaster.toSQLType(Caster.toString(arrTypes.get(i + 1, ""))), getRecordcount());
		}
		validateColumnNames(columnNames);
	}
