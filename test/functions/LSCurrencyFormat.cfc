component extends="org.lucee.cfml.test.LuceeTestCase" labels="currency" {
	...
	function beforeAll(){
		setLocale("en_us");
		variables.dt=CreateDateTime(2004,1,2,4,5,6);
		variables.euro=chr(8364);
	}
	...
	function run( testResults , testBox ) {
		describe( title="Test suite for LSCurrencyFormat()", body=function() {
			...
			it(title="checking LSCurrencyFormat() function, English (Australian)", body = function( currentSpec ) {
				...
				assertEquals("($1.00)", LSCurrencyFormat(-1));
				assertEquals("(1.00)", LSCurrencyFormat(-1, "none"));
				assertEquals("AUD(1.00)", replace(LSCurrencyFormat(-1, "international"),' ',''));
			});

			it(title="checking LSCurrencyFormat() function, German (Standard)", body = function( currentSpec ) {
				<!---
				German (Standard)) --->
				...
				assertEquals("100.000,00", LSCurrencyFormat(100000,"none"));
				assertEquals("(1,00)", LSCurrencyFormat(-1, "none"));
				assertEquals("EUR(1,00)", replace(LSCurrencyFormat(-1, "international"),' ',''));
				assertEquals("(1,00 #euro#)", LSCurrencyFormat(-1, "local"));

			});

			it(title="checking LSCurrencyFormat() function, German (Swiss)", body = function( currentSpec ) {
				<!---
				German (Swiss) --->
			...
			it(title="checking LSCurrencyFormat() function, German (Swiss)", body = function( currentSpec ) {
				...
				assertEquals("CHF(250.00)", replace(LSCurrencyFormat(-value,'international'),' ','','all'));
				assertEquals("250.00", LSCurrencyFormat(value,'none'));

			});

			it(title="checking LSCurrencyFormat() function, Portuguese (Brazilian)", body = function( currentSpec ) {
				...
				assertEquals("250,00", LSCurrencyFormat(value,'none'));
				assertEquals("(250,00)", replace(LSCurrencyFormat(-value,'none'),' ','','all'));
			});
		});
	}
