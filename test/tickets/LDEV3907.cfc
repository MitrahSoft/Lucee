component extends="org.lucee.cfml.test.LuceeTestCase" labels="orm" {
	...
	function run( testResults, testBox ) {
		describe("Testcase for LDEV-3907", function() {
			...
			it( title="updating the primary key in ORM entity and then accessing after saving (force:false)", skip=(noOrm() || notHasMssql()), body=function( currentSpec ) {
				try {
					local.result = _InternalRequest(
						template : "#uri#\LDEV3907.cfm",
						url: {
			...
			it( title="updating any value in the ORM entity and then accesssing after an ormsave (force: true)", skip=(noOrm() || notHasMssql()), body=function( currentSpec ) {
				try {
					local.result = _InternalRequest(
						template : "#uri#\LDEV3907.cfm",
						url: {
