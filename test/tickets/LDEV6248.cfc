component extends="org.lucee.cfml.test.LuceeTestCase" labels="config" {
	...
	function run( testResults, testBox ) {
		describe( "LDEV-6248: ConfigWebUtil.getFile NPE on invalid paths", function() {
			...
			it( "should not NPE when path has no parent (root path)", function() {
				...
				var result = ConfigWebUtil.getFile( config, configDir, "/", ResourceUtil.TYPE_FILE );
			...
			it( "should not NPE when path is a bare filename (no directory)", function() {
				...
				var result = ConfigWebUtil.getFile( config, configDir, "test.log", ResourceUtil.TYPE_FILE );
			});
			...
			it( "should not NPE when path is a URL-encoded placeholder", function() {
				...
				var result = ConfigWebUtil.getFile( config, configDir, "%7Blucee%2Dconfig%7D%2Flogs%2Fdatasource%2Elog", ResourceUtil.TYPE_FILE );
			});
			...
			it( "should resolve a valid placeholder path without error", function() {
				...
				var result = ConfigWebUtil.getFile( config, configDir, "{lucee-config}/logs/datasource.log", ResourceUtil.TYPE_FILE );
